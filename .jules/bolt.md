...
## 2026-08-31 - Optimize Room Sync with Batch Inserts for AiProfile
**Learning:** Blindly inserting/updating AI profiles in a loop using Room creates O(N) transaction overhead which severely degrades performance during sync.
**Action:** Pre-fetch existing entities, compare them, and accumulate changes in a list to use insertBatch and updateBatch inside a single transaction to significantly reduce SQLite churn and disk I/O.
