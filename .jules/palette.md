...
## 2024-05-18 - Tooltips for icon-only buttons\n**Learning:** Sighted users on Android benefit from `app:tooltipText` on icon-only buttons (like `ImageButton` and `FloatingActionButton`), which provides a tooltip when long-pressed, whereas `contentDescription` only serves screen readers.\n**Action:** Always add `app:tooltipText` matching the `contentDescription` on all icon-only interactive elements in Android XML layouts.
