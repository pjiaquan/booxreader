...
- **Feature:** Implemented comprehensive Authentication system:
    - **Auth Flow:** Email/Password Login, Registration, and Google Sign-In integration.
    - **Token Management:** `TokenManager` using `EncryptedSharedPreferences`, `AuthInterceptor` for Bearer tokens, and `TokenAuthenticator` for auto-refresh.
    - **Architecture:** Created `BooxReaderApp` for centralized dependency injection of `OkHttpClient` and `TokenManager`.
    - **Database:** Added `UserEntity` and `UserDao`.
    - **UI:** Added `LoginActivity`, `RegisterActivity`, and `UserProfileActivity`, accessible from Main screen.

## Setup Instructions for Google Sign-In
1. Go to Google Cloud Console/Firebase.
2. Create an Android Client ID using your keystore SHA-1.
3. Create a Web Client ID (for the backend token verification).
4. Replace `"YOUR_SERVER_CLIENT_ID"` in `LoginActivity.kt` with your Web Client ID.
